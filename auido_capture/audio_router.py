...
class AudioRouter:
    def __init__(
        ...
        """
        system_encoding/system_sample_rate: override the "system" (customer)
        channel's audio format. Defaults match the browser's tab-audio-share
        pipeline (PCM16 @ 16kHz). Twilio phone calls send mulaw @ 8kHz — pass
        system_encoding="mulaw", system_sample_rate=8000 for those sessions
        (see main.py's /ws/twilio handler). "mic" (agent) always stays
        browser PCM16 @ 16kHz regardless.
        """
